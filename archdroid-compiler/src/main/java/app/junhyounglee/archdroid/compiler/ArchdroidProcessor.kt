...
class ArchdroidProcessor : AbstractProcessor() {
    ...
    private fun setUpOptions(env: ProcessingEnvironment) {
        env.options.also { options ->
            ...
            options[OPTION_MIN_SDK]?.apply {
                try {
                    this@ArchdroidProcessor.sdk = Integer.parseInt(this)
    ...
    private fun setUpCoordinator(processingEnv: ProcessingEnvironment) {
        coordinators = ImmutableList.builder<ArchCoordinator>().run {
            add(MvpViewCoordinator(processingEnv))
            build()
        }
    }
    ...
    companion object {
        const val OPTION_MIN_SDK = "archdroid_min_sdk"
        const val OPTION_DEBUGGABLE = "archdroid_debuggable"
    }
}
...
//@AutoService(Processor::class)
//class ArchdroidProcessor : BasicAnnotationProcessor() {
//
//    private var sdk: Int = 1
//    private var debuggable: Boolean = false
//
...
//        setUpOptions(processingEnv)
//
//        processingEnv.mandatory("Archdroid> initiating Archdroid annotation processor.")
//        processingEnv.warning("Archdroid> initiating annotation processor.")
//
 ...
 * ViewGenerator
 */
abstract class ArchdroidObjectGenerator(
 ...
 *          , SampleView {
 */
class ViewGenerator(processingEnv: ProcessingEnvironment) : ArchdroidObjectGenerator(processingEnv) {
    ...
    override fun process(elementsByAnnotation: SetMultimap<Class<out Annotation>, Element>): Set<Element> {
        ...
        getSupportedAnnotations().forEach {
            ...
            for (elem in elements) {
                warning("Archdroid> ViewGenerator: parsing annotation: ${it.simpleName}", elem)
    ...
    private fun parseAndCreateTarget(annotation: Class<out Annotation>, element: Element) {
        ...
        warning("Archdroid> ViewGenerator.parseAndCreateTarget(), packageName: $packageName, simpleName: ${element.simpleName}")

        val annotatedType = element as TypeElement
        val enclosing = annotatedType.enclosingElement
        warning("Archdroid> ViewGenerator.parseAndCreateTarget(), qualifiedName: ${enclosing.kind}, enclosing element: ${enclosing.simpleName}")

        val baseView = element.getAnnotation(MvpActivityView::class.java)
        warning("Archdroid> ViewGenerator: base view: $baseView")

        element.enclosedElements.forEach {
            warning("Archdroid> enclosed(), ${getName(it)}")
        }
